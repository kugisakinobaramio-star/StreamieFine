...
export async function youTubeSearch(query, maxResults = 10) {
  ...
  return (Array.isArray(j) ? j : []).filter((v) => v.videoId).slice(0, maxResults).map((v) => ({
    ...
    title: v.title,
    author: v.author,
    length: v.lengthSeconds ? fmtDur(v.lengthSeconds) : '',
    views: v.viewCount ? fmtViews(v.viewCount) : '',
...
function fmtDur(s) {
  s = Number(s); const h = Math.floor(s / 3600), m = Math.floor((s % 3600) / 60), ss = s % 60;
...
export async function odyseeSearch(query, size = 12) {
  ...
  try {
    const r = await fetch(`https://lighthouse.odysee.com/search?s=${encodeURIComponent(query)}&size=${size}&from=0`, { signal: ctl.signal });
    if (!r.ok) throw new Error('HTTP ' + r.status);
    const j = await r.json();
    return (Array.isArray(j) ? j : []).filter((c) => c.name && c.claimId).map((c) => ({
      name: c.name,
      claimId: c.claimId,
      title: prettify(c.name),
      embed: `https://odysee.com/$/embed/${c.name}:${c.claimId}`,
      page: `https://odysee.com/${c.name}:${c.claimId}`,
    }));
  } finally { clearTimeout(t); }
}
function prettify(name) {
  return decodeURIComponent(name).replace(/[-_]+/g, ' ').replace(/\s+/g, ' ').trim().replace(/\b\w/g, (c) => c.toUpperCase()).slice(0, 90);
}

// ---- Rumble ----
// Rumble has no public search API, so: guided search deep-links + embed any pasted Rumble URL via oEmbed.
export const rumbleSearchUrl = (q) => `https://rumble.com/search/video?q=${encodeURIComponent(q)}`;
export async function rumbleEmbed(rawUrl) {
  const m = /rumble\.com\//.exec(rawUrl || '');
  if (!m) throw new Error('Not a Rumble URL');
  const r = await fetch(`https://rumble.com/api/Media/oEmbed.json?url=${encodeURIComponent(rawUrl)}`);
  if (!r.ok) throw new Error('oEmbed failed');
  const j = await r.json();
  const src = /src="([^"]+)"/.exec(j.html || '')?.[1];
  if (!src) throw new Error('No embed found');
  return { title: j.title, embed: src, author: j.author_name };
}

export function downloadUrl(url, filename) {
  const a = document.createElement('a');
  a.href = url;
  a.download = filename || 'streamiefine-video.mp4';
  a.target = '_blank';
  a.rel = 'noreferrer';
