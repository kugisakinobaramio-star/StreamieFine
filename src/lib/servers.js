...
function movieServers(tmdbId, imdbId) {
  const t=tmdbId,i=imdbId; return [
...
function seriesServers(tmdbId, imdbId, season, episode) {
  const t=tmdbId,i=imdbId,s=season??1,e=episode??1; return [
...
function animeServers(a,t,i,e,season=1) {
  const list=[]; if(a) list.push({id:'vidsrc-anime-sub',name:'Anime SUB',tag:'Sub',url:`https://vidsrc.cc/v2/embed/anime/${a}/${e||1}/sub`},{id:'vidsrc-anime-dub',name:'Anime DUB',tag:'Dub',url:`https://vidsrc.cc/v2/embed/anime/${a}/${e||1}/dub`});
  list.push(...seriesServers(t,i,season,e||1).map(s=>({...s,id:`tmdb-${s.id}`,name:`${s.name} (TMDB)`}))); return list.filter(s=>s.url);
}
export function getServersFor(item,{season,episode}={}) { if(!item)return[]; if(item.category==='anime')return animeServers(item.anilistId,item.tmdbId,item.imdbId,episode||1,season||1); if(item.rawType==='series'||item.category==='series'||item.category==='animation'||item.episodes)return seriesServers(item.tmdbId,item.imdbId,season||1,episode||1); return movieServers(item.tmdbId,item.imdbId); }
export function getPreferredServer(category){try{return localStorage.getItem('sf_srv_'+category)||null}catch{return null}}
export function setPreferredServer(category,serverId){try{localStorage.setItem('sf_srv_'+category,serverId)}catch{}}