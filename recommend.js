...
// "Because you watched X": same-category, genre-overlapping neighbours.
export function similarTo(item, pool, n = 10) {
  if (!item) return [];
  const gs = new Set(item.genres || []);
  return pool
    .filter((t) => t && t.id !== item.id && t.category === item.category)
